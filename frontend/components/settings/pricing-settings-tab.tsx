...
export function PricingSettingsTab() {
  ...
  const updateComboItem = (index: number, field: keyof ServiceComboItem, value: string | number) => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) => setZoneForm({ ...zoneForm, pricingType: e.target.value as "FIXED" | "PERCENTAGE" | "CUSTOM" })}
                    ...
                    {combo.items && combo.items.length > 0 && (
                      <div className="text-sm text-muted-foreground">
                        ...
                        <ul className="mt-1 space-y-0.5">
                          {combo.items.map((item: ServiceComboItem, idx: number) => (
                            <li key={idx}>• {item.quantity}x {(item as ServiceComboItem & { service?: { name: string } }).service?.name || item.serviceName}</li>
                          ))}
                        </ul>
                      </div>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) => setExceptionForm({ ...exceptionForm, exceptionType: e.target.value as "NO_FEE" | "CUSTOM_FEE" })}
                      className="flex h-9 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                    >
