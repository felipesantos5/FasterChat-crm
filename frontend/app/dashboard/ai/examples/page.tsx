  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2">Exemplos de conversas usados para treinar e melhorar as respostas da IA.</p>
      </div>
            ...
            <Card key={example.id} className="hover:shadow-md transition-shadow">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <CardTitle className="text-lg">{example.conversation.customer.name}</CardTitle>
                      <Badge variant="outline">{example.conversation.customer.phone}</Badge>
                    </div>
                    <CardDescription>Marcado em {formatDate(example.createdAt)}</CardDescription>
                    {example.notes && <p className="text-sm mt-2 text-muted-foreground italic">"{example.notes}"</p>}
                  </div>

                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => setSelectedExample(example)}>
                      <MessageSquare className="h-4 w-4 mr-2" />
                      Ver Conversa
                    </Button>
                    <Button size="sm" variant="destructive" onClick={() => handleRemove(example.conversationId)} disabled={removing}>
              ...
              <CardContent>
                <div className="flex items-center gap-4 text-sm text-muted-foreground">
                  <span>{example.conversation.messages.length} mensagens</span>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Dialog open={!!selectedExample} onOpenChange={() => setSelectedExample(null)}>
        <DialogContent className="max-w-3xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle>Conversa Exemplar - {selectedExample?.conversation.customer.name}</DialogTitle>
            <DialogDescription>{selectedExample?.notes && <span className="italic">"{selectedExample.notes}"</span>}</DialogDescription>
          </DialogHeader>
                ...
                return (
                  <div key={message.id} className={`flex ${isInbound ? "justify-start" : "justify-end"}`}>
                    ...
                    >
                      ...
                      <p className="text-xs mt-1 opacity-70">{format(new Date(message.timestamp), "HH:mm", { locale: ptBR })}</p>
                    </div>
                  </div>
                );
              })}
